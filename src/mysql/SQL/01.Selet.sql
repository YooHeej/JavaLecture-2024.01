...
SELECT * FROM city WHERE district IN ('Seoul', 'Inchon', 'Kyonggi');            /* 둘 다 같은 값 */
...
SELECT * FROM city WHERE district LIKE 'Chungchong%';             # % - 임의의 문자, 이 때는 LIKE 사용

/*
 *  1.2 순서(Order) - ASC(Ascending: 오름차순, default), DESC(Descending: 내림차순)
 */

 # 인구수가 900만 이상인 도시를 인구수의 내리차순으로 조회
 SELECT * FROM city WHERE population >= 9000000
...
SELECT * FROM city WHERE countrycode='KOR' 
    ORDER BY population DESC
    LIMIT 10 OFFSET 10;                 # 앞에서 10개를 제외한 10개를 보여줌.
...
# Aliasing
SELECT ROUND(AVG(population)) AS avgPop FROM city           # AS 생략 가능
...
#국내 도별(도시의 갯구가 2개 이상) 인구수의 평균을 내림차순으로
SELECT district, ROUND(AVG(population)) as avgPop FROM city
    WHERE countrycode='KOR'
    GROUP BY district
    HAVING count(*) >= 2                    # 그룹핑 조건 - 도시의 갯수가 2개 이상
...
# 국내 도시의 갯수가 5개 이상인 도의 평균 도시 인구수
...
-- SELECT countrycode, ROUND(AVG(population)) avgPop count(*) numCity FROM city
--     GROUP BY countrycode
--     HAVING count(*) >=100
--     ORDER BY avgPop DESC;
...
SELECT country.Name, city.Name, city.population FROM city
    INNER JOIN country ON city.countrycode=country.Code             # INNER 생략 가능
...
# 양쪽 테이블에서 필드명이 고유한 필드는 테이블 이름을 생략할 수 있다(Continent)
